...
class StreamTrainer(object):
    ...
    def test_net(self):
        ...
        for inputs, labels in self.dataloaders:
            ...
            torch.cuda.empty_cache()
            self.batch += 1
            print('Phase test | Current batch =', str(self.batch), '/', str(self.total_batch), end = '\r')
            self.update['progress'] = True
...
if __name__ == '__main__':
    ...
    j = """{
      ...
      "network": "i3d", 
      "output_compare": [], 
      "output_name": "Mixed3c_flow_charades",
      "patience": 10, 
      "pretrain_model": "i3d_flow_charades.pth.tar",
      "resize_h": 256, 
      "resize_w": 342, 
      "split": 1, 
      "step_size": 10, 
      ...
      "half_model" : "output/stream/UCF-101/split1/training/Mixed3c_flow_charades.pth.tar",
      "full_model": "output/stream/UCF-101/split1/training/Mixed3c_rgb_charades.pth.tar",
      "test_batch_size": 2,
      "test_subbatch_size": 20,
      "debug_test_size": 2
    }"""
